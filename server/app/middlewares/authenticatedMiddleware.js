...
exports.ensureAuth = function (req, res, next) {
    if (!req.headers.authorization)
        return res.status(406).send({ message: 'Authorization header not found.' });

    var token = req.headers.authorization.replace(/['"]+/g, '');
    try {
        var payload = jwt.decode(token, secret);
        if (payload.exp <= moment().unix())
            return res.status(406).send({ message: 'Expired  Token.' });

    } catch (err) {
        return res.status(406).send({ message: 'Token is not valid.' });
    }
    ...
    if (userId !== req.user.sub)
        return res.status(406).send({ message: 'User not valid.' });
        
    if (!req.user.twoFactorPassed)
        return res.status(406).send({ message: 'User not Authenticated' });
